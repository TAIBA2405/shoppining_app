...
import { TrendingUp, ShoppingBag, Package, Users, AlertTriangle, Clock, Star } from 'lucide-react'
    ...
    <div style={{ position: 'relative', height }}>
      <svg width="100%" height={height} style={{ overflow: 'visible' }}>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x} y={y} width={w} height={barH} rx="4" fill="url(#barGrad)" style={{ transition: 'all 0.4s ease' }} />
              <text x={`${i * barWidth + barWidth / 2}%`} y={height - 4} textAnchor="middle" fill="var(--text-tertiary)" fontSize="10" fontFamily="var(--font-body)">{d.label}</text>
              {d.value > 0 && <text x={`${i * barWidth + barWidth / 2}%`} y={y - 4} textAnchor="middle" fill="var(--color-primary)" fontSize="10" fontFamily="var(--font-body)" fontWeight="600">{d.value}</text>}
            </g>
          )
        })}
      </svg>
...
function DonutChart({ segments, size = 120 }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy - 6} textAnchor="middle" fill="var(--text-primary)" fontSize="18" fontWeight="800" fontFamily="var(--font-heading)">{total}</text>
      <text x={cx} y={cy + 12} textAnchor="middle" fill="var(--text-tertiary)" fontSize="10" fontFamily="var(--font-body)">orders</text>
    </svg>
  )
}

const STATUS_COLORS = {
  placed: '#60a5fa', confirmed: '#c9a84c', shipped: '#fbbf24',
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{ textAlign: 'center', color: 'var(--text-tertiary)' }}>
          <div style={{ width: 36, height: 36, border: '3px solid var(--border-color)', borderTopColor: 'var(--color-primary)', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
  ...
  return (
    <div id="admin-dashboard">
      ...
      <div className="admin-kpi-grid">
        {kpis.map((kpi, i) => (
          <motion.div key={kpi.label} className="admin-kpi-card" style={{ '--kpi-color': kpi.color }}
      ...
      <div className="admin-charts-row">
        <motion.div className="admin-chart-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} style={{ marginBottom: 0 }}>
        ...
        <motion.div className="admin-chart-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} style={{ marginBottom: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {statusBreakdown.length === 0 && <div style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-xs)', textAlign: 'center' }}>No orders yet</div>}
      ...
      <div className="admin-bottom-row">
        <motion.div className="admin-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <div className="admin-card-header">
            <span className="admin-card-title">Top Products</span>
            <Link to="/products" style={{ fontSize: 'var(--text-xs)', color: 'var(--color-primary)', textDecoration: 'none' }}>View all &rarr;</Link>
          </div>
          {topProducts.length === 0
            ? <div style={{ padding: '32px 20px', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--text-sm)' }}>No sales data yet</div>
            : <div style={{ padding: 12 }}>
                {topProducts.map(({ product, count }, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', borderRadius: 'var(--radius-md)' }}>
                    ...
                    {product.images?.[0] && <img src={product.images[0]} alt={product.name} style={{ width: 36, height: 44, objectFit: 'cover', borderRadius: 6 }} />}
        ...
        <motion.div className="admin-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
          <div className="admin-card-header">
                  ...
                  <tbody>
                    {orders.slice(0, 8).map(order => (
                      <tr key={order.id}>
                        ...
                        <td>
                          <span className="admin-status-badge" style={{ background: `${STATUS_COLORS[order.status] || '#888'}18`, color: STATUS_COLORS[order.status] || '#888' }}>
                            {order.status}
                          </span>
                        </td>
                      </tr>
